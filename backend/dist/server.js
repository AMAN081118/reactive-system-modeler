...
app.post("/api/validate", (req, res) => {
    try {
        ...
        for (const transition of stateMachine.transitions) {
            const fromExists = stateMachine.states.some((s) => s.id === transition.from);
        ...
        for (const transition of stateMachine.transitions) {
            ...
            if (transition.input && inputs.has(transition.input)) {
                errors.push(`Duplicate input "${transition.input}" on state ${stateMachine.states.find((s) => s.id === transition.from)?.name ||
                    transition.from}`);
            }
            if (transition.input) {
...
app.post("/api/simulate", (req, res) => {
    try {
        const { stateMachine, inputs } = req.body;
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            const currentState = stateMachine.states.find((s) => s.id === currentStateId);
            if (!currentState) {
                isValid = false;
                break;
            }
            // Find matching transition
            const transition = stateMachine.transitions.find((t) => t.from === currentStateId && t.input === input);
            if (!transition) {
                isValid = false;
                steps.push({
                    step: i,
...
app.post("/api/generate-tests", (req, res) => {
    try {
        ...
        while (queue.length > 0) {
            ...
            const outgoing = stateMachine.transitions.filter((t) => t.from === stateId);
...
app.post("/api/verify", (req, res) => {
    try {
        ...
        if (initialStates.length === 1) {
            ...
            while (queue.length > 0) {
                ...
                if (!reachableStates.has(curr)) {
                    ...
                    const outgoing = stateMachine.transitions.filter((t) => t.from === curr);
        ...
        // Report summary
        const summary = `States: ${stateMachine.states.length} (Reachable: ${reachableStates.size}) | Transitions: ${stateMachine.transitions.length} | Status: ${errors.length === 0 ? "VALID" : "INVALID"}`;
        res.json({
            success: true,
            data: {
